...
if $cygwin || $msys ; then
    ...
    if [ "$__CYGPATTERN" ] ; then
        OURCYGPATTERN="($OURCYGPATTERN|($__CYGPATTERN))"
    fi

    # Now convert the arguments via cygpath
    for i do
        arg="$i"
        case $arg in                        # convert to --arg=value
            --*) set -- "$@" "$arg"
                continue ;;
            -I?*) set -- "$@" -I"`echo "$arg" | sed 's,^-[I]\(.*\),\1,'`"
                continue ;;
            -[il]) set -- "$@" "$arg"
                continue ;;
                # eat a space
            --) set -- "$@" "$arg"
                continue ;;
            -[dD][rR][wW]?*) set -- "$@" -D"`echo "$arg" | sed 's/^-[dD][rR][wW]\(.*\)/\1/'`"
                continue ;;
            --* | -*) ;;
            *)
                # Added quotes to allow spaces in path names
                arg=`echo "$arg" | sed "s|^$OURCYGPATTERN|$APP_HOME|g"`
                arg="${arg//\\\\/ /}"
                arg="${arg% }"
                set -- "$@" "$arg"
                ;;
